...
const caseStudyData = {
    'Shop in 3D': {
        pages: [
            {
                title: 'The Problem',
                text: 'The pandemic shut down Microsoft retail stores, severing the ability for potential customers to physically connect with Surface products before purchasing. Static 2D image galleries failed to communicate build quality. The initial business requirement was to create a 3D screen-sharing tool for Customer Service agents, but we soon realized the UX needed to be intuitive enough for a completely self-service customer experience.<br><br>The biggest threat was scope creep. With multiple organizations (Store, Surface, MSCOM, Sales) heavily invested, I had to pivot my role from just designing UI to actively defending the core user experience against a barrage of competing feature requests.'
            },
            {
                title: 'The Constraints',
                text: 'We initially explored using external vendors, but their platforms failed to meet the granularity of detail we knew we would want beyond the MVP. I made the strategic call to bring the architecture in-house using Babylon.js to give us granular control over rendering and system resources.<br><br>Embedding a 3D canvas into a standard webpage also introduced severe UX friction, most notably the "scroll trap" where users would accidentally zoom into a model instead of scrolling down the page. I had to design custom zoom limits, cursor "grab" affordances, and a full-screen breakout mode to protect the traditional shopping flow. On the performance side, I manually optimized 8GB CAD manufacturing models down to a strict 5MB limit, ensuring the experience loaded instantly on 4G networks.'
            },
            {
                title: 'The Iteration',
                text: 'To prove the concept without crashing browsers, I had to ruthlessly prioritize. I worked with our engineer to strip out the Chatbot, pricing, and universal headers for the Proof of Concept, focusing purely on core sales scenarios like checking ports and screen sizes. I also engineered a DOM hierarchy that allowed keyboard tabbing through the 3D space, ensured high-visibility focus states, and changed "hotspots" to screen-reader-friendly "POIs".<br><br>To handle the entire Surface lineup without memory leaks, I prototyped loading mechanics using AssetContainers to cache and dynamically swap meshes. My coded prototype ultimately served as the sole source of truth for the engineering team.'
            },
            {
                title: 'The Outcome',
                text: 'By managing system resources in-house and relentlessly focusing on the core UX, the prototype thrived during user testing with sales associates. It proved the concept so effectively that leadership gained the confidence to expand the tool from top-sellers to the entire Surface lineup. That specific prototype justified the $2 million in funding for the MVP and ultimately contributed to a 3% increase in Surface sales. I delivered a scalable, functional system that solved the immediate business need and provided the engineering team with a bulletproof blueprint.'
            }
        ],
        images: [
    ...
    'Project Mio': {
        pages: [
            {
                title: 'The Problem',
                text: 'Traditional coding education was failing to engage kids. It was viewed as isolated "dark room coding"—repetitive, intimidating, and boring. The business requirement was to create an educational tool that could demystify programming and help address the looming shortage of software developers. We realized that forcing middle schoolers to stare at raw syntax errors would only push them away. Our mandate was to build a web-based game that was fun first and educational second, wrapping fundamental JavaScript concepts in familiar, highly engaging gameplay mechanics.'
            },
            {
                title: 'The Constraints',
                text: 'We weren\'t just building a theoretical prototype; we had a hard deadline to present a functional build to 650 middle schoolers at a live assembly. The game had to be accessible via standard web browsers and run smoothly on low-powered school Chromebooks without choking the local network or our Azure servers. To hit our aggressive three-phase milestone plan (POC, MVP, Beta), we couldn\'t rely on traditional, slow-moving corporate silos. Our small team had to establish a rapid-iteration workflow using GitHub, Slack, and OneDrive to bridge the gap between art direction, UX, and engineering.'
            },
            {
                title: 'The Iteration',
                text: 'To bridge the gap between gaming and coding, I designed a system called "Snippet Cards." Recognizing how kids interact with trading card games, and how real developers pull modular code from sites like Stack Overflow, these cards allowed players to collect, drag, and drop code chunks into an execution window. Instead of a blank editor, players solved platforming puzzles by modifying the world\'s actual properties. We deployed this MVP live to 7th and 8th graders. Watching them actively engage with the Snippet Cards and navigate the JavaScript logic validated that we had successfully gamified the syntax.'
            },
            {
                title: 'The Outcome',
                text: 'The live deployment was a massive success, generating unsolicited fan art and requests for multiplayer features from the students. We successfully proved that modular, gamified education could lower the barrier to entry for computer science. More importantly, the highly collaborative, cross-functional workflow our small strike team developed to build the game was so efficient that leadership took notice. Our workflow ultimately became the blueprint used to restructure the wider organization into agile, self-reliant "Pods."'
            }
        ],
        images: [
